...
is_unsupported = platform.python_implementation() == "PyPy" or sys.version_info < (
    3,
    10,
)
...
def remove_platform_dependent(s):
    ...
    while i < len(s):
        ...
        if v.strip().startswith("float16"):
            ...
            v.startswith("def ret_numpy_half()")
            or v.startswith("def test_slots()")
            or v.startswith("TypeAlias")
        ):
            i += 2
        else:
            s2.append(v)
...
def test01_check_stub_refs(p_ref, request):
    ...
    if "test_functions_ext" in p_in.name and sys.version_info < (3, 13):
        s_ref = [
            line.replace("types.CapsuleType", "typing_extensions.CapsuleType")
            for line in s_ref
        ]
        s_ref.insert(3, "")
    ...
    diff = list(
        difflib.unified_diff(s_ref, s_in, fromfile=str(p_ref), tofile=str(p_in))
    )
    if len(diff):
        for p in diff:
