...
def test41_kw_only():
    ...
    assert t.test_kw_only_all.__doc__ == "test_kw_only_all(*, i: int, j: int) -> tuple"
    ...
    assert (
        t.test_kw_only_mixed.__doc__ == "test_kw_only_mixed(i: int, *, j: int) -> tuple"
    )
...
def test45_new():
    ...
    assert t.test_bytearray_new("\x00\x01\x02\x03", 4) == bytearray(b"\x00\x01\x02\x03")
            ...
            if expect_ret is None:
                ...
                if hasattr(sys, "getrefcount"):
                    ...
                    if py_3_14a7_or_newer:
                        assert (
                            ret_refs == expected_refs - 1 or ret_refs == expected_refs
                        )
                    else:
