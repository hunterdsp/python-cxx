...
def test01_metadata():
    ...
    assert (
        not t.check_float(np.array([1], dtype=np.bool_))
        and not t.check_float(np.array([1], dtype=np.uint32))
        and t.check_float(np.array([1], dtype=np.float32))
    )
    assert (
        not t.check_bool(np.array([1], dtype=np.uint32))
        and not t.check_bool(np.array([1], dtype=np.float32))
        and t.check_bool(np.array([1], dtype=np.bool_))
    )
...
def test02_docstr():
    ...
    assert (
        t.pass_float32.__doc__ == "pass_float32(array: ndarray[dtype=float32]) -> None"
    )
    assert (
        t.pass_complex64.__doc__
        == "pass_complex64(array: ndarray[dtype=complex64]) -> None"
    )
    assert t.pass_bool.__doc__ == "pass_bool(array: ndarray[dtype=bool]) -> None"
    assert (
        t.pass_float32_shaped.__doc__
        == "pass_float32_shaped(array: ndarray[dtype=float32, shape=(3, *, 4)]) -> None"
    )
    assert (
        t.pass_float32_shaped_ordered.__doc__
        == "pass_float32_shaped_ordered(array: ndarray[dtype=float32, shape=(*, *, 4), order='C']) -> None"
    )
    assert t.check_device.__doc__ == (
        "check_device(arg: ndarray[device='cpu'], /) -> str\n"
...
def test26_return_ro():
    ...
    assert (
        t.ret_numpy_const_ref.__doc__
        == "ret_numpy_const_ref() -> numpy.ndarray[dtype=float32, shape=(2, 4), order='C', writable=False]"
    )
    assert (
        t.ret_numpy_const_ref_f.__doc__
        == "ret_numpy_const_ref_f() -> numpy.ndarray[dtype=float32, shape=(2, 4), order='F', writable=False]"
    )
    assert x.shape == (2, 4)
    assert y.shape == (2, 4)
    assert np.all(x == [[1, 2, 3, 4], [5, 6, 7, 8]])
...
def test49_test_matrix4f_ref():
    assert (
        t.Matrix4f.data_ref.__doc__.replace("data_ref", "data")
        == t.Matrix4f.data.__doc__
    )
